...
func Mkfs(id string, type_ string, fs_ string) {
	...
	Prints.Prints = append(Prints.Prints, "Fs: "+fs_)
	...
	copy(newSuperblock.S_mtime[:], fechaFormateada)
	copy(newSuperblock.S_umtime[:], fechaFormateada)
	newSuperblock.S_mnt_count = 0

	if fs_ == "2fs" {
		create_ext2(n, TempMBR.Partitions[index], newSuperblock, fechaFormateada, file)
	} else {
		fmt.Println("EXT3")
	}
...
func create_ext2(n int32, partition Structs.Partition, newSuperblock Structs.Superblock, date string, file *os.File) {
	Prints.Prints = append(Prints.Prints, "\n======Start CREATE EXT2======")
	Prints.Prints = append(Prints.Prints, "N: "+string(n))
	fmt.Println("Superblock:", newSuperblock)
	Prints.Prints = append(Prints.Prints, "Date: "+date)
	...
	Prints.Prints = append(Prints.Prints, "======End CREATE EXT2======")
